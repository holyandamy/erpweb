...
function Datepicker() {
	...
	this.regional[''] = { // Default regional settings
		...
		dayNamesMin: ['��','һ','��','��','��','��','��'], // Column headings for days starting at Sunday
...
$.extend(Datepicker.prototype, {
	...
	_dialogDatepicker: function(input, date, onSelect, settings, pos) {
		...
		if (!inst) {
			...
			$('.cander-box').append(this._dialogInput);//body
	...
	*/
	...
	_showDatepicker: function(input) {
		...
		inst.dpDiv.css({position: ($.datepicker._inDialog && $.blockUI ?
			'static' : (isFixed ? 'fixed' : 'static')), display: 'none',
			left: 350 + 'px', top: 480 + 'px'});
		if (!inst.inline) {
	...
	_updateDatepicker: function(inst) {
		...
		$(".ui-datepicker-calendar").find("a").each(function(){
			...
			if($targetVal.indexOf(action) >= 0){
				$(this).css({background:"#3ec3c8",color:"#fff"});
	...
	_hideDatepicker: function(input) {
		...
		if (this._datepickerShowing) {
			...
			if (this._inDialog) {
				...
				if ($.blockUI) {
					$.unblockUI();//cander-box
					//$('body').append(this.dpDiv);
					$('.cander-box').append(this.dpDiv);
				}
			}
...
$.fn.datepicker = function(options){
	...
	if (!$.datepicker.initialized) {//body
		$(document).mousedown($.datepicker._checkExternalClick).
			find('.cander-box').append($.datepicker.dpDiv);
